...
| P10 | **Adotar o modelo de inimigo do Gaiadon** (jul/05, pós-playtest) — nível do mob ACOMPANHA o jogador dentro da banda da área (`clamp(player, area.min, area.max)`) + stats pela fórmula `(nível/x)^y` com gap de expoente (parede) | revisa a IMPLEMENTAÇÃO do P1 (mob deixa de ser preso ao piso; passa a subir junto com você, mas ainda preso à ÁREA) e SUPERA o P2b (some o "entra-derrete"; vira desafio ~constante que a parede de expoente endurece). Spec: `GAIADON_ENEMY_MODEL.md`. Re-fit multi-sessão | o dono jogou e o "mob congelado Lv 1" incomodou; a Gaiadon math mostrou que mob=área é do gênero, mas o mob deve SUBIR com o jogador dentro da banda (não prender no piso) — resolve o feel sem jogar fora o mundo de áreas |
