...
> Última atualização: **2026-07-04** (maratona: P9 v8 baked · tela de passivas completa 16/16 · reforma arquitetural em 5 lotes · skills Matt Pocock instaladas user-level).

---

## Onde o trabalho está AGORA

**SESSÃO jul/04 (a mais recente) — três frentes fechadas:**

1. **P9 REBALANCE ✅ BAKED (v8 em `src/`)** — spec completa em `docs/design/P9_REBALANCE.md`. Substitui o relógio P0/P8.5b: **First Light ~36h±2** (37h29–58 nas seeds), curva foguete (acelera→desacelera), Okhra 71–95 golpes, crit real 44.5% (≤45-50 travado), composição ATK gear+passivas 83–91% (≥75 travado), re-subida ≤2.9 (aspiracional ≤2 pendente), awaken materials como massa (30k–100k), executionerCap 8%, Overcrit removido do Mapa 1 (vira tier 2 no Mapa 2). 6 mecânicas de assinatura novas (Cleave, Bulwark, Overcrit†, Momentum, Golden Wake, Executioner's Light). **Falta: playtest humano rodada 3 (dono, v8) → travar P9 ou rodada 4.**
2. **TELA DE PASSIVAS (World-Tree I) ✅ COMPLETA** — arte full-bleed (`passives_tree.webp`), 16/16 tiles instalados (`pv_1..pv_16`, cada passiva com identidade cromática própria, formato TILE edge-to-edge), galhos orgânicos "seiva de luz" (Bézier + taper + 3 estados), painel **Tree Blessings** (soma agregada dos bônus), tooltip acessível 440px, nameplate tipográfico (✦ World Tree ✦ / PASSIVES / Tier I). Pipeline de sprite 1-a-1 com o dono documentado nos commits.
3. **REFORMA ARQUITETURAL ✅ (5 lotes, jul/04)** — ~53 MB de assets órfãos deletados · bug "NIHELIM"→Okhra corrigido · save resiliente a quota · código morto removido · `combat.js` decomposto (rates/enemyFactory/income/progression) · `ui.js` MORTO, decomposto em 9 módulos `ui-*` (ordem de scripts documentada no CLAUDE.md §Stack). Provas: sim byte-idêntico, 313 asserções ALL PASS, reviews adversariais. Event-bus combat↔ui ficou de fora de propósito (Mapa 2). `okhra_red.png` preservado (decisão de arte estacionada).

**Skills do Matt Pocock instaladas user-level** (`~/.claude/skills`, 25 skills; clone em `~/.claude/mattpocock-skills`). Próxima sessão: rodar `/setup-matt-pocock-skills` 1x, e o dono quer **`/grill-with-docs` sobre `docs/design/GAIADON_NUMBERS.md`** — objetivo declarado: dominar o balanceamento e definir o que falta pro Mapa 1 ser jogável por terceiros. Agenda já rascunhada (nesta ordem): playtest rodada 3 → onboarding de estranho → copy de fronteira (item 4) → P8.6 retunes.
...
**A ESCADA P0–P8(+P8.5b) ESTÁ COMPLETA** — balance do Mapa 1 travado, implementado e revisado de ponta a ponta (`docs/design/BALANCE_FRAMEWORK_MAP1.md`). **⚠️ ATUALIZAÇÃO jul/04: o P9 SUBSTITUIU o relógio e várias âncoras abaixo (18h → ~36h±2). Onde P0–P8.5b divergirem do `P9_REBALANCE.md`, o P9 vence.**
...
0. **PRÓXIMO DA FILA: playtest rodada 3 do P9 (dono, v8 no jogo) → travar P9 ou rodada 4.** Depois: `/grill-with-docs` do Gaiadon + agenda "jogável por terceiros" (ver topo). Nota jul/04: a arte do Tema B avançou muito (map2.png + medalhões node_10-18 + mobs do Porto commitados em sessões jul/03-04); conferir o que resta do bestiário antes de reabrir esta fila. Item original:
0a. ~~(design): arte do bestiário do Tema B~~ **(parcialmente feito — ver nota acima).** Lore do Porto ✅ travada (jul/03) — bestiário canon existe (`mapa1_tema_b_porto_afundado.md` §4, 10 Vessels com buscas Pinterest). Ordem de produção: referências do dono por mob → prompts → mobs + H4/H5/H6 → backgrounds 10–18 (imagens impossíveis no §3) → arte do mapa Ato B (placeholder azulado no código espera `assets/ui/map2.png`) → finale (Okhra, design travado, refs instaladas). Junto com cada lote de arte: renames/blurbs/rosters do `data.js` (§7 do doc, via Sonnet).
