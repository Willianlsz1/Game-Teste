...
// =============================================================
// Apenas INFRAESTRUTURA — sem balanceamento. Requisitos CONFIGURÁVEIS
// (data.awakens[].requirements); os números são placeholders. Cada Awaken
...
G.awaken = {
  ...
  isUnlocked(id) { return this.isDone(id); },

  // valor "alcançado" pelo jogador para cada chave de requisito
  playerValue(key) {
    const d = G.state.data;
    switch (key) {
      case "area": return (d.maxAreaUnlocked || 0) + 1;
      case "level": return d.level || 1;
      case "kills": return d.totalKills || 0;
  ...
  canUnlock(id) { return this.canAwaken(id); },
  ...
  unlock(id) { return this.awaken(id); },
